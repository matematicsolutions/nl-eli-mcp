...
Pulls a tiny static ``nl-runtime.json.gz`` from this repo's GitHub Release on
first tool call so a dead endpoint can be repointed (edit runtime/nl-runtime.json
...
Opt-out: set ``NL_ELI_RUNTIME_URL=""`` to disable the fetch.
"""
...
MANIFEST_URL = os.environ.get(
    "NL_ELI_RUNTIME_URL",
    "https://github.com/matematicsolutions/nl-eli-mcp/releases/latest/download/nl-runtime.json.gz",
)
_CACHE_NAME = "nl-runtime.json"
_TIMEOUT = 15
_USER_AGENT = "nl-eli-mcp-runtime (+https://github.com/matematicsolutions/nl-eli-mcp)"

_runtime: dict[str, Any] | None = None


def _cache_dir() -> Path:
    env = os.environ.get("NL_ELI_CACHE_DIR")
    if env:
        return Path(env).expanduser()
    return Path.home() / ".matematic" / "cache" / "nl-eli"
