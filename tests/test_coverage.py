...
from nl_eli_mcp.coverage import build_coverage
from nl_eli_mcp.server import nl_coverage


def _coverage():
    fn = getattr(nl_coverage, "fn", nl_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NL_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("nl_coverage" in line for line in lines), (
        "an audit entry was written but it does not name nl_coverage"
    )
